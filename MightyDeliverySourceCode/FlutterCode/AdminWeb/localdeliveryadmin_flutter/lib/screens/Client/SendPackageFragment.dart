...
class SendPackageFragmentState extends State<SendPackageFragment> {
  ...
  Future<void> init() async {
    ...
    if (widget.orderId != null) {
      await orderDetail(orderId: widget.orderId!).then((value) {
        ...
        if (value.data!.pickupPoint!.contactNumber.validate().split(" ").length == 1) {
          pickPhoneCont.text = value.data!.pickupPoint!.contactNumber.validate().split(" ").last;
        } else {
          pickupCountryCode = value.data!.pickupPoint!.contactNumber.validate().split(" ").first;
          pickPhoneCont.text = value.data!.pickupPoint!.contactNumber.validate().split(" ").last;
        }
        ...
        if (value.data!.deliveryPoint!.contactNumber.validate().split(" ").length == 1) {
          deliverPhoneCont.text = value.data!.deliveryPoint!.contactNumber.validate().split(" ").last;
        } else {
          deliverCountryCode = value.data!.deliveryPoint!.contactNumber.validate().split(" ").first;
          deliverPhoneCont.text = value.data!.deliveryPoint!.contactNumber.validate().split(" ").last;
        }
  ...
  Future<List<Predictions>> getPlaceAutoCompleteApiCall(String text) async {
    ...
    await placeAutoCompleteApi(searchText: text, language: 'en', countryCode: CountryData.fromJson(getJSONAsync(COUNTRY_DATA)).code.validate(value: 'IN')).then((value) {
      ...
      child: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            16.height,
            if (ResponsiveWidget.isSmallScreen(context)) Column(children: [mWeight(), 16.height, mNomOfParcel()]) else Row(children: [mWeight().expand(), 16.width, mNomOfParcel().expand()]),
            16.height,
                ...
                children: [
                  ...
                  Row(
                    ...
                    children: [
                      ...
                      mHeadingValue('${weightController.text} ${CountryData.fromJson(getJSONAsync(COUNTRY_DATA)).weightType}'),
                    ],
                  ),
                  16.height,
                  Row(
                                          ...
                                          InkWell(
                                            onTap: () {
                                              ...
                                              if (selectedTabIndex != 3) {
                                                ...
                                                showConfirmDialogCustom(context, title: language.createOrderConfirmation, onAccept: (buildContext) {
                                                  createOrderApiCall(ORDER_CREATE);
                                                }, primaryColor: primaryColor, dialogType: DialogType.CONFIRMATION);
                                              }
                                            },
                                            child: Container(
                                              alignment: Alignment.center,
